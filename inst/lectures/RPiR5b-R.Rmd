...
Remember, `whole` was `seq(from = 3, to = 6)`:
...
accidentally delete useful things if it’s used in the wrong place. If you have
to, you can clear the environment manually using *Session->Clear Workspace*, the
brush in the Environment tab, or even better use *Session->Restart R* to restart
everything. Next:
```{R, eval=FALSE}
...
if you ever move your code. Much better to use a Project, so please don't put
this into your code anywhere.
...
t make sure it is done as your code is downloaded), but your scripts should
