...
```{r eval = FALSE}
demo(topic = "d0301_run_birth_death", package = "package0301")
```
...
all of the demos installed as part of the {package0301} package. 
...
When you add your own demos, you need to run (`devtools::load_all()` to 
temporarily load or) `devtools::install()` to permanently install them as part 
of your package.
...
visible. You need to run (`devtools::load_all()` to 
temporarily load or) `devtools::install()` to permanently install them as part 
of your package.
...
<span style="color: #de77ae;">man/</span> files and edit the <span style="color: #de77ae;">NAMESPACE</span>. You should then run (`devtools::load_all()` to 
temporarily load or) `devtools::install()` to permanently install them as part 
of your package.
...
used to record package dependencies. We've added a dependency on {dplyr} so you
...
<span style="color: #de77ae;">package0301-package</span> file
* Run `devtools::document()` to populate the <span style="color: #de77ae;">NAMESPACE</span> file (which you should never edit by hand)

You should then run (`devtools::load_all()` to 
temporarily load or) `devtools::install()` to permanently install them as part 
of your package.
...
particular analysis. The format is usually `major.minor.patch`, see [here](https://semver.org) for more details. 
