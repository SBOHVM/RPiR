---
title: "Reproducible Programming in R Practical 1-1"
output: 
  learnr::tutorial:
    theme: lumen
...
so that they all get repeated. Try it and see:
...
<span style="color: #de77ae;">Compile Report...</span>. An html document should 
pop up in a viewer after a couple of seconds, and a new file should appear in 
the directory you were working in. The lines below the yaml header, beginning 
with `#'` generate text chunks in such a report. 
...
mathematically but it is good practice to use **give things meaningful names** 
in a computer program, so we are going to use the name `growth.rate` instead. We 
have also switched to use the correct values for human demographic growth, 
rather than the simple numbers we used earlier. 
...
this will define a section in our report. Our simple model earlier only gave us 
the final population size, but we want to be able to plot the population size 
...
is working correctly. You should see that `population.vector` has a length of 1 
and a value of 7e+09.
...
Now we should specify the sequence of timesteps:
...
`tail(population)` would give the last 5 values in the vector, but by specifying 
...
quiz(caption = "Quiz",
     question("How many new births are there in time step 3?",
     ...
     question("In which time step do new births exceed 120,000,000?",
              answer("11"),
              answer("8"),
...
<span style="color: #de77ae;">0101-growth-functional.r</span> (check the header 
...
different sections if necessary. When running code in R we have the option of
...
```{r summary, exercise = TRUE, exercise.setup = "plot"}
...
```{r head, exercise = TRUE, exercise.setup = "plot"}
# Print the first 5 elements of population.vector
...
```{r head2, exercise = TRUE, exercise.setup = "plot"}
# Print the first 10 elements of population.vector

```

```{r head2-solution}
# Print the first 10 elements of population.vector
head(population.vector, 10)
```
...
```{r tail, exercise = TRUE, exercise.setup = "plot"}
...
```{r tail-solution}
# Print the last element of population.vector
summary(tail(population.vector, 1))
```
...
Finally, try copying the code into an *.R document in R Studio, saving it, and
generating a report from it (<span style="color: #de77ae;">File</span> >
<span style="color: #de77ae;">Compile Report...</span>). You'll generate your 
own reports in Practical 2.
