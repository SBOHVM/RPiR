...
description: "Reproducible Programming in R Practical 1-1!"
---
...
own copy of `ggplot()` or `lmer()` in every script you do, so why should you 
...
Download files 3 and 4, the full census data and the taxonomic data on all of 
the species present, and download the 
...
The critical test is that you can restart R, call `library(xxx)` on your
package, and then immediately access the data that you created. If you are 
testing someone else’s you’ll need to download it first by creating a new 
project based on the GitHub repository or by calling
...
Once the package(s) is working, you need to submit it/them as your assignment
for today. First you need to make sure you have pushed all of the changes to 
GitHub, then add me, <span style="color: #1697C0;">richardreeve</span>, as a
collaborator on the repository on GitHub, and then submit the link to the GitHub 
web page for the repository as you exercise on GitHub. If you’d like to write a 
short script demonstrating it working, you can add an R file to the repository,
but make sure (confusingly!) not to put it in the R subfolder, or the package 
will very likely not build. Check this by 
<span style="color: #1697C0;">Install and Restart</span>ing the package before
you submit it!
