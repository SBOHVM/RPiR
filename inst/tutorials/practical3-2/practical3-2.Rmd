...
<span style="color: #de77ae;">demo/stochastic-birth-death.r</span>, 
remembering to add an entry in 
<span style="color: #de77ae;">demo/00Index</span>. You can use the 
<span style="color: #de77ae;">run-birth-death</span> demo (simulating 
birth-death population dynamics from Practical 1-5) as a template. 
...
Now you will need to write the `timestep.stochastic.birth.death()` 
function (in <span style="color: #de77ae;">0301-stochastic-birth-death.r</span>) 
to provide the right information to the main loop. We already have a 
deterministic step function for single species population 
dynamics (Practical 1-5). Starting with this code, you need to make three 
changes. First, we need to handle the effect of changing timesteps and update
the code from Practical 1-5 (or the code you produced for SIS and SIR models in
Practicals 2-3 and 2-4) with something like this:
