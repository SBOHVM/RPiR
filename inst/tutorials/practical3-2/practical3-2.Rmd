---
title: "3-2: Stochastic population growth model"
output: 
  learnr::tutorial:
    theme: lumen
...
fact, something has. There is now the chance that the population will die out before 
the end of the experiment, so we need to look for this happening – not least to 
speed up the simulation.

### demo/d0302_stochastic_birth_death.R

We’ll start by writing the structural code in a new demo file, 
<span style="color: #de77ae;">demo/d0302_stochastic_birth_death.r</span>, 
remembering to add an entry in 
<span style="color: #de77ae;">demo/00Index</span>. You can use the 
<span style="color: #de77ae;">d0301_run_birth_death</span> demo -- simulating 
birth-death population dynamics from Practical 1-5 -- in {package0301} as a 
template. 
...
Remember to call `library(RPiR)` at the top of the script to load
`plot_populations()`, and `library(package0301)` to load `timestep_stochastic_birth_death()`, which means you 
...
function to provide the right information to the main loop. We already have a 
deterministic step function for single species population 
dynamics (from Practical 1-5). We've written this function into {package0301} as
`step_deterministic_birth_death()`. Starting with this code, you need to make three 
changes. 
...
timestep_stochastic_birth_death <- function(latest, birth.rate, death.rate, timestep) {
  effective.birth.rate <- birth.rate * timestep
  # -- some code to determine the numbers of new births and deaths --
  next.population <- data.frame(time = latest$time + timestep, # -- some arguments -- )
  # -- some code to calculate whether the experiment is finished --
...
To test your function works you can run `load_all()` from the {devtools} 
package to load your package into your Global environment. 

Once you're happy with your function, run `document()`, again from the {devtools} 
package, to populate your package documentation files. Then run `install()`
to install it.
...
the output. Remember that you can run your code as a demo, 

```{r eval = FALSE}
demo("d0302_stochastic_birth_death", package = "package0301")
```

or by opening the file in RStudio and compiling a notebook. 

The stochasticity means that we see variable outputs. If you don’t
