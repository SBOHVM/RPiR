...
before needs to be adapted -- Shannon entropy, which 
is $\log(D_1)$, and the Berger-Parker index, which is $\frac{1}{D_\infty}$, and adapt the code to cope with these.
...
entropy and the Berger-Parker index, using the generic function you created 
...
$\log(D_1)$ and Berger Parker is $\frac{1}{D_\infty}$ – as detailed above in 
...
where $p$ is the vector of non-zero species proportions.
