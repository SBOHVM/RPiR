...
description: "Reproducible Programming in R Practical 2-1!"
---
...
<span style="color: #1697C0;">File > New project > New Directory > Empty 
Project</span>, and then choose a location and a name. Then create a new R file, 
which I do through <span style="color: #1697C0;">File > New > R Script</span>. 
...
You should save the file as **0201-growth-loop.r**. We have included numbers in 
our file names for convenience as the first four digits will correspond to the 
practical number you are currently on. 
...
we used earlier. The code should now look like this:
...
languages. But **don’t use** spaces or arithmetic symbols (*e.g.* the minus 
sign). 
...
You should see a little triangle to the left of `for (new.time in timesteps) {`. 
...
menus and select <span style="color: #1697C0;">Code -> Reindent Lines</span>. 
...
```{r try-summary, exercise = TRUE}
summary(population.vector)
```

```{r try-head1, exercise = TRUE}
head(population.vector)
```

```{r try-head2, exercise = TRUE}
head(population.vector, 3)
```

```{r try-tail1, exercise = TRUE}
tail(population.vector,1)
```

```{r try-tail2, exercise = TRUE}
...
<span style="color: #1697C0;">Compile Notebook...</span> and choose the 
<span style="color: #1697C0;">html</span> output format and press 
<span style="color: #1697C0;">Compile</span> if necessary. A webpage should pop 
up in a viewer after a couple of seconds, and a new file should appear in the 
directory you are working in called, **0201-growth-loop.html**. You can check 
