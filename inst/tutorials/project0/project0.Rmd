...
which constitutes 33% of the marks, so well-structured and well-commented code, 
...
repository in the IBAHCM organisation as the last two practical series.**
...
stating what you have just done. You are welcome to do more commits if this is
useful  to you, and it's completely okay (and indeed normal) if you go back to
earlier exercises and make further edits and commit them after completing later 
exercises. 
...
- Functions should be kept in the <span style="color: #1697C0;">R</span> folder
...
- Scripts for each project exercise should be stored in the
  <span style="color: #1697C0;">demo</span> folder. They should be available
...
1. Create an initial empty R package called **githubusernameDiversity** in
   RStudio that uses git and is connected to a private repository in the IBAHCM
   organisation;
   - You will use this package for all of your remaining work in this project
2. Update the <span style="color: #1697C0;">DESCRIPTION</span> file with the
   correct information about yourself and the package;
3. Create a help file for the package in the
   <span style="color: #1697C0;">R</span> folder, and run `devtools::document()`
...
4. Add your data package as a dependency of the package (or mine if you're using
   it instead) -- the `usethis::use_dev_package()` function will add the correct
   entries to the <span style="color: #1697C0;">DESCRIPTION</span> file, just
...
5. Import your data package into the package by adding the `@import` command to
   the package's help file in the <span style="color: #1697C0;">R</span> folder
   that you made in step 3, above;
6. Commit these changes and push them to GitHub.

You will need to much of this again in subsquent project exercises, for
...
[RPiR website](https://ibahcm.github.io/RPiR/articles/pages/packages_guide.html#add-dependencies).
...
```
it edits your <span style="color: #1697C0;">DESCRIPTION</span> file to add the
CRAN `ggplot2` package to your package's dependencies and the `RPiR` package in
the IBAHCM organisation on GitHub to your package's dependencies as a package
...
`devtools::install_github("IBAHCM/RPiR")` or `devtools::install()` locally). If
you then add the following to the documentation in the
<span style="color: #1697C0;">R</span> folder, this will mark the functions and
