...
code. The function documentation should be in the function file now and not 
...
and the main script (<span style="color: #de77ae;">0103-run-simple-growth.r</span>) should look something like this:
...
in the <span style="color: #de77ae;">Environment</span> tab in 
RStudio (or <span style="color: #de77ae;">Session > Clear Workspace...</span> in the menus). All 
...
leaving RStudio itself unaffected. Remember 
that once you have done this you may 
...
As before, generate a report from the script file and check that it produces the 
same results as before. Note 
...
report from the function file! You can make it so the report will fail to 
generate for that script when global variables are used with the following code 
snippets:
