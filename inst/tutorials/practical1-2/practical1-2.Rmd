---
title: "1-2: Exponential growth model"
output:
  learnr::tutorial:
    theme: lumen
    includes:
...
description: >
  Students learn how to write a function and how to differentiate between `<-`
...
```{r inout, exercise = TRUE, exercise.lines = 11}
...
we did on line 28 of <span style="color: #de77ae;">0101-growth-functional.r
</span>. 
...
Here's the latest version of the code to play with (<span style="color: #de77ae;">0102-growth-functional.r</span>), complete with the 
...
Find all the instances of `growth.rate` and replace them with `my_rate` in the 
...
function now only has an argument called `rate` (line 11). To fix this, we can 
change the call to `step_simple_growth()` to say `my_rate = growth.rate` (line 
...
with `my_population`, or something sensible, then run the code. What happens? You 
