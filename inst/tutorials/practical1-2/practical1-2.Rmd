...
starting each analysis script with the R command `library(mypackage)`, where
`mypackage` is the name of the package you created. 
...
```{r eval = FALSE}
install.packages(c("vegan", "tibble", "dplyr", "magrittr", "devtools"))
devtools::install_github("boydorr/rdiversity")
```

Note the second of these commands. Since the {rdiversity} package has been 
updated since the last version was submitted to CRAN, we can use 
`install_github()` to install the latest development version from GitHub. 
...
in your demo script (*i.e.* you call `library(pkgname)` somewhere) should go 
into the <span style="color: #de77ae;">DESCRIPTION</span> file.

Now we're going to create a demo inside the package. First go to the 
<span style="color: #de77ae;">Files</span> tab and create a new folder called
...
(this file will contain a list of all demos in your package). Each demo you 
...
beta and gamma diversity, detailed below. But in order to do some work with 
...
created the package `{mypackage}`, and in it `bci_taxa` contains the taxonomy of 
...
`dist2sim()` to create a similarity matrix from the distance matrix. So:
...
the axes of the abundance data matrix. So:
...
https://github.com/boydorr/rdiversity (click on the docs badge) and / or in R. 
...
reinstall the package, call `library(mypackage)` and run 
...
`demo("diversity-of-BCI", package = "mypackage")` without loading the library 
first.
