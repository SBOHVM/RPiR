...
`character(0)`, which is code speak for exit_success). Next note that the object 
names `new.additions` and `next.population` remain internal to the function 
...
the function and an object in the main script, so we ended up setting one to 
