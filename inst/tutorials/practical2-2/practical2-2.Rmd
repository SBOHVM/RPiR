...
When we look at our first program (from Practical 2-1) there are several
...
affect the running of the code. For clarity, however, and general readability 
of the code we'll rename these objects later (don't worry about them right now).
...
```{r nextstep, exercise = TRUE, exercise.lines = 42}
#' Set up the simulation parameters
#' --------------------------------
#' First we set up the parameters for the simulation.
...
`<-` in the main script as you did in 1-1. `growth.rate` is both an
...
```{r laststep, exercise = TRUE, exercise.lines = 42}
#' step_simple_growth function
step_simple_growth <- function(current.population, growth.rate) {
  # Calculate changes to population
  new.additions <- growth.rate * current.population
...
This is because when you call the function (on line 38), you set its 
`growth.rate` argument to the main script’s value of `growth.rate`, but the 
function now only has an argument called `rate1` (line 2). To fix this, we can 
change the call to `step_simple_growth()` to say `rate1 = growth.rate` (on line 
38). Now rerun the code and check it works.
